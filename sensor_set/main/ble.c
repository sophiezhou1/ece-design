...
/*
 * Barebones BLE telemetry example.
 *
 * NOTE:
 * - This file is intentionally separate so your current main.c is untouched.
 * - To use it, swap this filename into idf_component_register(SRCS ...)
 *   and add REQUIRES bt nvs_flash in sensor_set/main/CMakeLists.txt.
 */
...
/*
 * Replace this function body with your existing sensor_set data path:
 * - thermocouple_c: from MAX31855 thermocouple decode
 * - food_probe_c: from ADC conversion path
 * - control_flags: your runtime control state bitfield
 * - tc_fault_flags: OC/SCG/SCV bits
 */
static sensor_packet_t collect_sensor_packet(void)
{
    ...
    // Placeholder values so the example runs immediately.
    pkt.thermocouple_c = 72.5f;
    pkt.food_probe_c = 68.1f;
    pkt.control_flags = 0x03;
    pkt.tc_fault_flags = 0x00;
